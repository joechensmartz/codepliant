...
/**
 * Documents that are typically published on a website and should NOT contain
 * internal management metadata (Document Version/Owner/Next Review Date),
 * Related Documents links, Review Notes, or "Generated by Codepliant" footers.
 */
const USER_FACING_DOCS = new Set([
  "PRIVACY_POLICY.md",
  "TERMS_OF_SERVICE.md",
  "COOKIE_POLICY.md",
  "AI_DISCLOSURE.md",
...
export function generateDocuments(
  ...
  const complianceEvidenceLog = generateComplianceEvidenceLog(docScan, ctx);
  if (complianceEvidenceLog) {
    docs.push({
      name: "Compliance Evidence Log",
      filename: "COMPLIANCE_EVIDENCE_LOG.md",
      content: complianceEvidenceLog,
    });
  }

  // Privacy Impact Screening — quick screening to determine if full DPIA needed
  const privacyImpactScreening = generatePrivacyImpactScreening(docScan, ctx);
  if (privacyImpactScreening) {
    docs.push({
  ...
  // End User License Agreement — always generated for any software product
  docs.push({
  ...
  // Append Review Notes and Related Documents sections to internal docs only.
  // Skip user-facing documents (privacy policy, terms, cookie policy, etc.)
  // as these sections reference internal files users don't have on their website.
  for (const doc of docs) {
    // Skip user-facing documents — they should not contain internal references
    if (USER_FACING_DOCS.has(doc.filename)) {
      continue;
    }
  ...
  // into internal documents only. User-facing documents (privacy policy, terms, etc.)
  // should not contain internal management metadata.
  ...
  for (const doc of docs) {
    // Skip JSON files, user-facing docs, and docs that already have Document Version
    if (doc.filename.endsWith(".json") || doc.content.includes("Document Version") || USER_FACING_DOCS.has(doc.filename)) {
      continue;
    }
  ...
  // Remove "Generated by Codepliant" footer from user-facing docs by default.
  // Only keep it if config has showPoweredBy: true.
  if (!ctx.showPoweredBy) {
    for (const doc of docs) {
      if (USER_FACING_DOCS.has(doc.filename)) {
        // Remove the footer pattern: "---\n\n*This ... generated by [Codepliant](...) ...*"
        doc.content = doc.content.replace(
          /\n---\n\n\*This[^*]*generated by \[Codepliant\]\([^)]*\)[^*]*\*\s*$/,
          ""
        );
        // Also remove inline "Generated by [Codepliant]" blockquote lines
        doc.content = doc.content.replace(
          /^> .*Generated.*\[Codepliant\].*\n/gm,
          ""
        );
      }
    }
