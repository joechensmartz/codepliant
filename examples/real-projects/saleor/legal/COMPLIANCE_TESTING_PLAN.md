...
| --- | --- | --- | --- |
| DD-S-001 | **boto3** | Data fully removed after deletion request | List objects by user prefix; expect empty |
| DD-S-002 | **Django Channels** | Data fully removed after deletion request | Service-specific API query; expect no user data |
| DD-S-003 | **django-admin** | Data fully removed after deletion request | Query user profile API; expect 404 |
| DD-S-004 | **django-sessions** | Data fully removed after deletion request | Query user profile API; expect 404 |
| DD-S-005 | **redis** | Data fully removed after deletion request | Direct DB query by user ID; expect 0 rows |
| DD-S-006 | **stripe** | Data fully removed after deletion request | Query payment records by user ID; expect empty result |

### 3.3 Cascade Deletion

| ID | Test Case | Expected Result | Regulation |
| --- | --- | --- | --- |
| DD-C-001 | Delete user account | Data removed from all 6 detected services | GDPR Art. 17 |
...
- **django-admin**: Verify MFA enforcement for admin accounts
- **django-admin**: Verify OAuth scope restrictions match configured minimums
- **django-admin**: Verify session token rotation on privilege escalation
- **django-sessions**: Verify MFA enforcement for admin accounts
- **django-sessions**: Verify OAuth scope restrictions match configured minimums
- **django-sessions**: Verify session token rotation on privilege escalation
...
| --- | --- | --- | --- | --- |
| **boto3** | N/A | Required | Required | Required |
| **Django Channels** | N/A | Required | Required | Recommended |
| **django-admin** | N/A | Required | Required | Required |
| **django-sessions** | N/A | Required | Required | Required |
