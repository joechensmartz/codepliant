...
This guide provides actionable engineering patterns for implementing privacy by design across the [Your Company Name] technology stack. Each section is tailored to the 6 service(s) detected in the codebase and includes code examples, configuration recommendations, and testing strategies.
...
| --- | --- | --- | --- | --- |
| **boto3** | storage | Classify and tag PII objects; auto-expire temporary uploads | SSE (AES-256); enforce HTTPS-only access | Bucket policies; no public access; pre-signed URLs |
| **Django Channels** | other | Review data collected; document purpose and retention | TLS in transit; encrypt PII at rest | Principle of least privilege; document access grants |
| **django-admin** | auth | Collect only required profile fields; limit OAuth scopes | Encrypt tokens and credentials at rest; TLS in transit | Admin-only config; audit all auth events |
| **django-sessions** | auth | Collect only required profile fields; limit OAuth scopes | Encrypt tokens and credentials at rest; TLS in transit | Admin-only config; audit all auth events |
...
async function deleteUserData(userId: string): Promise<DeletionReport> {
  ...
  const deletionTasks = [
    { service: "boto3", fn: () => deleteboto3Data(userId) },
    { service: "Django Channels", fn: () => deleteDjangoChannelsData(userId) },
    { service: "django-admin", fn: () => deletedjangoadminData(userId) },
    { service: "django-sessions", fn: () => deletedjangosessionsData(userId) },
