...
> 23 services | Generated on 2026-03-16
...
|---------|----------|---------------|-----------|-------------|-------------------|
| @hubspot/api-client | other | **P2** | **P3** | contact information, email addresses, names | Assess per data processed |
| @sendgrid/mail | email | **P1** | **P2** | email addresses, email content | CAN-SPAM, GDPR Art. 33/34 |
| @sentry/nextjs | monitoring | **P2** | **P2** | error data, stack traces, user context | Minimal direct regulatory impact |
| @upstash/redis | database | **P0** | **P1** | cached data, session data | GDPR Art. 33/34, data breach notification |
| Google Analytics | analytics | **P1** | **P3** | page views, user behavior, device information | ePrivacy, GDPR Art. 6/7 (consent) |
| Google Tag Manager | analytics | **P1** | **P3** | page views, user behavior, custom events | ePrivacy, GDPR Art. 6/7 (consent) |
| google-auth-library | auth | **P0** | **P1** | OAuth tokens, Google profile data, email | GDPR Art. 33/34, credential breach notification |
| googleapis | other | **P2** | **P3** | user data via Google APIs, calendar data, email data | Assess per data processed |
| intercom | other | **P2** | **P3** | user profiles, email, name | Assess per data processed |
| ioredis | database | **P0** | **P1** | cached data, session data | GDPR Art. 33/34, data breach notification |
| next-auth | auth | **P0** | **P1** | email, name, profile picture | GDPR Art. 33/34, credential breach notification |
| nodemailer | email | **P1** | **P2** | email addresses, email content | CAN-SPAM, GDPR Art. 33/34 |
| passport | auth | **P0** | **P1** | email, name, OAuth tokens | GDPR Art. 33/34, credential breach notification |
| Plausible Analytics | analytics | **P1** | **P3** | page views, referrer data, device information | ePrivacy, GDPR Art. 6/7 (consent) |
...
| prisma | database | **P0** | **P1** | user data as defined in schema | GDPR Art. 33/34, data breach notification |
| Redis | database | **P0** | **P1** | session data, cache data | GDPR Art. 33/34, data breach notification |
| Redis (env) | database | **P0** | **P1** | session data, cache data | GDPR Art. 33/34, data breach notification |
| stripe | payment | **P0** | **P1** | payment information, billing address, email | PCI DSS, GDPR Art. 33/34 |
| twilio | other | **P2** | **P3** | phone numbers, SMS message content, voice call metadata | Assess per data processed |
| web-push | other | **P2** | **P3** | push subscription endpoints, device tokens, notification content | Assess per data processed |
...
*This incident severity matrix was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on an automated scan of the **calcom-monorepo** codebase (23 services detected). Customize response times, escalation paths, and roles to match your organization's structure. This document should be reviewed by your security team and legal counsel before adoption.*
