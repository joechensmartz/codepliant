...
|---------|---------------|
| google-auth-library | OAuth tokens, Google profile data, email |
| next-auth | email, name, profile picture, OAuth tokens, session data |
| passport | email, name, OAuth tokens, session data |
...
Consider integrating TOTP-based MFA (e.g., via google-auth-library) as a first step.
