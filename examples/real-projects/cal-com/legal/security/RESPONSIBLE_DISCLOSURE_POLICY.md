...
| Authentication system (google-auth-library, next-auth, passport) | Auth Infrastructure | Critical |
| Payment processing (stripe) | Payment Infrastructure | Critical |
| Data storage (@upstash/redis, ioredis, PostgreSQL, PostgreSQL (env), prisma, Redis, Redis (env)) | Data Infrastructure | High |
...
- **Authentication & Authorization** — bypass, privilege escalation, session management flaws (google-auth-library, next-auth, passport)
- **Payment Security** — transaction manipulation, price tampering, payment data exposure (stripe)
- **Data Storage** — injection, unauthorized data access, data leakage (@upstash/redis, ioredis, PostgreSQL, PostgreSQL (env), prisma, Redis, Redis (env))
