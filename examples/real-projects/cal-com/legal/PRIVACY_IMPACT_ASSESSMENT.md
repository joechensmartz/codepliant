...
This assessment covers the data processing activities of the **calcom-monorepo** application operated by [Your Company Name]. The following describes the nature, scope, context, and purposes of processing.
...
|---------|----------|---------------|-------------|
| @hubspot/api-client | other | contact information, email addresses, names, phone numbers, company data, deal information, engagement history | Legitimate interest |
| @sendgrid/mail | email | email addresses, email content | Contractual necessity / Consent |
| @sentry/nextjs | monitoring | error data, stack traces, user context, device information, IP address, performance profiles | Legitimate interest |
| @upstash/redis | database | cached data, session data | Contractual necessity |
| Google Analytics | analytics | page views, user behavior, device information, IP address, location data | Consent |
| Google Tag Manager | analytics | page views, user behavior, custom events, device information, third-party tag data | Consent |
| google-auth-library | auth | OAuth tokens, Google profile data, email | Contractual necessity |
| googleapis | other | user data via Google APIs, calendar data, email data, profile information | Legitimate interest |
| intercom | other | user profiles, email, name, conversations, user behavior, company data | Legitimate interest |
...
| Plausible Analytics | analytics | page views, referrer data, device information | Consent |
| PostgreSQL | database | application data, user records | Contractual necessity |
...
| twilio | other | phone numbers, SMS message content, voice call metadata, call recordings | Legitimate interest |
| web-push | other | push subscription endpoints, device tokens, notification content | Legitimate interest |

### 1.3 Categories of Personal Data

- **Personal Identity Data:** Email addresses, names, profile pictures, and account credentials collected through authentication. (sources: next-auth, google-auth-library, passport)
...
- **Usage & Behavioral Data:** Page views, click patterns, session recordings, device information, and IP addresses collected through analytics tools. (sources: posthog, Google Analytics, Google Tag Manager, Plausible Analytics)
- **Communication Data:** Email addresses and email content processed through email service providers. (sources: nodemailer, @sendgrid/mail)
...
- **Stored User Data:** Persistent user data stored in databases as defined by the application schema. (sources: ioredis, @upstash/redis, prisma, PostgreSQL (env), Redis (env), PostgreSQL, Redis, Redis (Cache), lru-cache, @prisma/extension-accelerate)
- **API Data Collection:** 20 API endpoint(s) accepting user data via MUTATION, QUERY requests. Data fields collected: autoOptIn, color, eventTypeIds, feature, featureId, includeSystemRolesOnly, isAll, name, permissions, roleId, routingFormId, slug, surveyId, teamId, userId. (sources: packages/trpc/server/routers/features/_router.ts, packages/trpc/server/routers/viewer/eventTypes/_router.ts, packages/trpc/server/routers/viewer/featureOptIn/_router.ts, packages/trpc/server/routers/viewer/feedback/_router.ts, packages/trpc/server/routers/viewer/insights/_router.ts, packages/trpc/server/routers/viewer/pbac/_router.tsx)
...
|---------------------|-------------|---------------|
| Data processing via @hubspot/api-client | Legitimate interest | To be documented based on specific use case |
...
| Data persistence via @upstash/redis | Contractual necessity | Necessary to persist user data for service delivery |
| Behavioral analytics via Google Analytics | Consent | Used to understand usage patterns and improve the service; requires prior consent |
| Behavioral analytics via Google Tag Manager | Consent | Used to understand usage patterns and improve the service; requires prior consent |
| Authentication via google-auth-library | Contractual necessity | Necessary to authenticate and maintain user sessions |
| Data processing via googleapis | Legitimate interest | To be documented based on specific use case |
| Data processing via intercom | Legitimate interest | To be documented based on specific use case |
...
| Behavioral analytics via Plausible Analytics | Consent | Used to understand usage patterns and improve the service; requires prior consent |
| Data persistence via PostgreSQL | Contractual necessity | Necessary to persist user data for service delivery |
...
| Data processing via twilio | Legitimate interest | To be documented based on specific use case |
| Data processing via web-push | Legitimate interest | To be documented based on specific use case |
...
|---|---------------------|---------------|------------|--------|-------|--------|
| 1 | Data processing via @hubspot/api-client | contact information, email addresses, names, phone numbers, company data, deal information, engagement history | 1 (Unlikely) | 3 (Significant) | **3** | **Low** |
| 2 | Email communications via @sendgrid/mail | email addresses, email content | 2 (Possible) | 2 (Limited) | **4** | **Low** |
| 3 | Error/performance monitoring via @sentry/nextjs | error data, stack traces, user context, device information, IP address, performance profiles | 2 (Possible) | 2 (Limited) | **4** | **Low** |
| 4 | Data persistence via @upstash/redis | cached data, session data | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
| 5 | Behavioral analytics via Google Analytics | page views, user behavior, device information, IP address, location data | 3 (Likely) | 2 (Limited) | **6** | **Medium** |
| 6 | Behavioral analytics via Google Tag Manager | page views, user behavior, custom events, device information, third-party tag data | 3 (Likely) | 2 (Limited) | **6** | **Medium** |
| 7 | Authentication via google-auth-library | OAuth tokens, Google profile data, email | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
...
| 9 | Data processing via intercom | user profiles, email, name, conversations, user behavior, company data | 1 (Unlikely) | 2 (Limited) | **2** | **Low** |
| 10 | Data persistence via ioredis | cached data, session data | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
| 11 | Authentication via next-auth | email, name, profile picture, OAuth tokens, session data | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
| 12 | Email communications via nodemailer | email addresses, email content | 2 (Possible) | 2 (Limited) | **4** | **Low** |
| 13 | Authentication via passport | email, name, OAuth tokens, session data | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
| 14 | Behavioral analytics via Plausible Analytics | page views, referrer data, device information | 3 (Likely) | 1 (Negligible) | **3** | **Low** |
| 15 | Data persistence via PostgreSQL | application data, user records | 2 (Possible) | 1 (Negligible) | **2** | **Low** |
| 16 | Data persistence via PostgreSQL (env) | application data, user records | 2 (Possible) | 1 (Negligible) | **2** | **Low** |
| 17 | Behavioral analytics via posthog | user behavior, session recordings, feature flag usage, device information | 3 (Likely) | 3 (Significant) | **9** | **High** |
| 18 | Data persistence via prisma | user data as defined in schema | 2 (Possible) | 1 (Negligible) | **2** | **Low** |
| 19 | Data persistence via Redis | session data, cache data | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
| 20 | Data persistence via Redis (env) | session data, cache data | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
| 21 | Payment processing via stripe | payment information, billing address, email, transaction history | 2 (Possible) | 4 (Maximum) | **8** | **Medium** |
| 22 | Data processing via twilio | phone numbers, SMS message content, voice call metadata, call recordings | 1 (Unlikely) | 3 (Significant) | **3** | **Low** |
| 23 | Data processing via web-push | push subscription endpoints, device tokens, notification content | 1 (Unlikely) | 1 (Negligible) | **1** | **Low** |
...
### 6.1 Other (@hubspot/api-client, googleapis, intercom, twilio, web-push)
...
### 6.2 Email (@sendgrid/mail, nodemailer)
...
### 6.4 Database (@upstash/redis, ioredis, PostgreSQL, PostgreSQL (env), prisma, Redis, Redis (env))
...
### 6.5 Analytics (Google Analytics, Google Tag Manager, Plausible Analytics, posthog)
...
### 6.6 Auth (google-auth-library, next-auth, passport)
