...
This guide provides actionable engineering patterns for implementing privacy by design across the [Your Company Name] technology stack. Each section is tailored to the 23 service(s) detected in the codebase and includes code examples, configuration recommendations, and testing strategies.
...
- **@sentry/nextjs**: Configure `beforeSend` / scrubbing hooks to strip PII from error payloads
...
- **@upstash/redis**: Enforce SSL/TLS on all connections (`sslmode=require` or equivalent)
...
| --- | --- | --- | --- | --- |
| **@hubspot/api-client** | other | Review data collected; document purpose and retention | TLS in transit; encrypt PII at rest | Principle of least privilege; document access grants |
| **@sendgrid/mail** | email | Store only transactional addresses; purge marketing lists | TLS for SMTP; encrypt stored templates with PII | Marketing team only for campaigns; devs for transactional |
| **@sentry/nextjs** | monitoring | Scrub PII from error payloads and breadcrumbs | TLS for log shipping; encrypt log archives | Read-only dashboards; admin for config changes |
| **@upstash/redis** | database | Encrypt PII columns; implement column-level access | TDE or volume encryption; TLS for connections | Per-service DB users; no shared credentials |
| **Google Analytics** | analytics | Anonymize IPs; use aggregated metrics over user-level | TLS in transit; anonymize before storage | Read-only for analysts; no raw PII export |
| **Google Tag Manager** | analytics | Anonymize IPs; use aggregated metrics over user-level | TLS in transit; anonymize before storage | Read-only for analysts; no raw PII export |
| **google-auth-library** | auth | Collect only required profile fields; limit OAuth scopes | Encrypt tokens and credentials at rest; TLS in transit | Admin-only config; audit all auth events |
...
| **intercom** | other | Review data collected; document purpose and retention | TLS in transit; encrypt PII at rest | Principle of least privilege; document access grants |
...
| **passport** | auth | Collect only required profile fields; limit OAuth scopes | Encrypt tokens and credentials at rest; TLS in transit | Admin-only config; audit all auth events |
| **Plausible Analytics** | analytics | Anonymize IPs; use aggregated metrics over user-level | TLS in transit; anonymize before storage | Read-only for analysts; no raw PII export |
...
| **twilio** | other | Review data collected; document purpose and retention | TLS in transit; encrypt PII at rest | Principle of least privilege; document access grants |
| **web-push** | other | Review data collected; document purpose and retention | TLS in transit; encrypt PII at rest | Principle of least privilege; document access grants |
...
**Detected:** Google Analytics, Google Tag Manager, Plausible Analytics, posthog
...
**Detected:** @sendgrid/mail, nodemailer
...
async function deleteUserData(userId: string): Promise<DeletionReport> {
  ...
  const deletionTasks = [
    { service: "@hubspot/api-client", fn: () => deletehubspotapiclientData(userId) },
    { service: "@sendgrid/mail", fn: () => deletesendgridmailData(userId) },
    { service: "@sentry/nextjs", fn: () => deletesentrynextjsData(userId) },
    { service: "@upstash/redis", fn: () => deleteupstashredisData(userId) },
    { service: "Google Analytics", fn: () => deleteGoogleAnalyticsData(userId) },
    { service: "Google Tag Manager", fn: () => deleteGoogleTagManagerData(userId) },
    { service: "google-auth-library", fn: () => deletegoogleauthlibraryData(userId) },
    { service: "googleapis", fn: () => deletegoogleapisData(userId) },
    { service: "intercom", fn: () => deleteintercomData(userId) },
    ...
    { service: "passport", fn: () => deletepassportData(userId) },
    { service: "Plausible Analytics", fn: () => deletePlausibleAnalyticsData(userId) },
    ...
    { service: "twilio", fn: () => deletetwilioData(userId) },
    { service: "web-push", fn: () => deletewebpushData(userId) },
  ];
