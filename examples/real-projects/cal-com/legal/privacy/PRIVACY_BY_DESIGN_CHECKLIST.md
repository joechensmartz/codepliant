...
| --- | --- | --- | --- |
| @hubspot/api-client | other | contact information, email addresses, names, phone numbers, company data, deal information, engagement history | Review data processing scope; assess DPA requirements |
| @sendgrid/mail | email | email addresses, email content | Consent/opt-out mechanism; DPA with provider; retention policy |
| @sentry/nextjs | monitoring | error data, stack traces, user context, device information, IP address, performance profiles | PII scrubbing in error reports; retention limits; access controls |
| @upstash/redis | database | cached data, session data | Encryption at rest; access controls; backup encryption |
| Google Analytics | analytics | page views, user behavior, device information, IP address, location data | Consent mechanism; IP anonymization; retention review |
| Google Tag Manager | analytics | page views, user behavior, custom events, device information, third-party tag data | Consent mechanism; IP anonymization; retention review |
| google-auth-library | auth | OAuth tokens, Google profile data, email | Minimize data collected; secure credential storage; session management |
...
| intercom | other | user profiles, email, name, conversations, user behavior, company data | Review data processing scope; assess DPA requirements |
...
| passport | auth | email, name, OAuth tokens, session data | Minimize data collected; secure credential storage; session management |
| Plausible Analytics | analytics | page views, referrer data, device information | Consent mechanism; IP anonymization; retention review |
...
| twilio | other | phone numbers, SMS message content, voice call metadata, call recordings | Review data processing scope; assess DPA requirements |
| web-push | other | push subscription endpoints, device tokens, notification content | Review data processing scope; assess DPA requirements |
