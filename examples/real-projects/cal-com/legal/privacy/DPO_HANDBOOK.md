...
- 23 third-party data processors detected (systematic processing at scale)
...
### 5.2 Systems to Query (23 detected)

- **@hubspot/api-client** (other): contact information, email addresses, names
- **@sendgrid/mail** (email): email addresses, email content
- **@sentry/nextjs** (monitoring): error data, stack traces, user context
...
- **intercom** (other): user profiles, email, name
- **ioredis** (database): cached data, session data
- **next-auth** (auth): email, name, profile picture
- **nodemailer** (email): email addresses, email content
- **passport** (auth): email, name, OAuth tokens
...
- **twilio** (other): phone numbers, SMS message content, voice call metadata
- **web-push** (other): push subscription endpoints, device tokens, notification content

## 6. Payment Data Responsibilities
...
*This DPO Handbook was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on an automated scan of the **calcom-monorepo** codebase. It should be reviewed and customized by your legal team and appointed DPO before adoption.*
