...
This guide provides actionable engineering patterns for implementing privacy by design across the [Your Company Name] technology stack. Each section is tailored to the 11 service(s) detected in the codebase and includes code examples, configuration recommendations, and testing strategies.
...
| **@segment/analytics-next** | analytics | Anonymize IPs; use aggregated metrics over user-level | TLS in transit; anonymize before storage | Read-only for analysts; no raw PII export |
...
| **algoliasearch** | other | Review data collected; document purpose and retention | TLS in transit; encrypt PII at rest | Principle of least privilege; document access grants |
| **cookie-parser** | other | Review data collected; document purpose and retention | TLS in transit; encrypt PII at rest | Principle of least privilege; document access grants |
| **express-session** | other | Review data collected; document purpose and retention | TLS in transit; encrypt PII at rest | Principle of least privilege; document access grants |
...
| **Multer** | storage | Classify and tag PII objects; auto-expire temporary uploads | SSE (AES-256); enforce HTTPS-only access | Bucket policies; no public access; pre-signed URLs |
...
**Detected:** @segment/analytics-next, posthog
...
**Detected:** @aws-sdk/client-s3, Multer
...
async function deleteUserData(userId: string): Promise<DeletionReport> {
  ...
  const deletionTasks = [
    ...
    { service: "@segment/analytics-next", fn: () => deletesegmentanalyticsnextData(userId) },
    { service: "@sendgrid/mail", fn: () => deletesendgridmailData(userId) },
    { service: "algoliasearch", fn: () => deletealgoliasearchData(userId) },
    { service: "cookie-parser", fn: () => deletecookieparserData(userId) },
    { service: "express-session", fn: () => deleteexpresssessionData(userId) },
    { service: "ioredis", fn: () => deleteioredisData(userId) },
    { service: "Multer", fn: () => deleteMulterData(userId) },
