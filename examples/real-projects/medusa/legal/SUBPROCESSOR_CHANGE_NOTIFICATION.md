...
| Segment (Twilio) | Product analytics and usage tracking | user identity, user behavior, page views, custom events, device information, IP address |
| SendGrid (Twilio) | Transactional and marketing email delivery | email addresses, email content |
| Algolia | Third-party service integration | search queries, search result clicks, user search behavior |
| cookie-parser | Third-party service integration | cookies, cookie data |
| express-session | Third-party service integration | session cookies, session data |
| Multer | File storage and media hosting | uploaded files, file metadata, potential PII in uploaded content |
