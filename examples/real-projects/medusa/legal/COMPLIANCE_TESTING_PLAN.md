...
| CF-A-@SE | **@segment/analytics-next**: Verify no tracking before consent | No network requests to @segment/analytics-next endpoints until consent given | GDPR Art. 7 |
| CF-A-@SE2 | **@segment/analytics-next**: Verify tracking stops on withdrawal | @segment/analytics-next tracking ceases within 1 page load of withdrawal | GDPR Art. 7(3) |
...
| --- | --- | --- | --- |
| DD-S-001 | **@aws-sdk/client-s3** | Data fully removed after deletion request | List objects by user prefix; expect empty |
| DD-S-002 | **@segment/analytics-next** | Data fully removed after deletion request | Export user data report; expect no matching records |
| DD-S-003 | **@sendgrid/mail** | Data fully removed after deletion request | Search mailing lists by email; expect not found |
| DD-S-004 | **algoliasearch** | Data fully removed after deletion request | Service-specific API query; expect no user data |
| DD-S-005 | **cookie-parser** | Data fully removed after deletion request | Service-specific API query; expect no user data |
| DD-S-006 | **express-session** | Data fully removed after deletion request | Service-specific API query; expect no user data |
| DD-S-007 | **ioredis** | Data fully removed after deletion request | Direct DB query by user ID; expect 0 rows |
| DD-S-008 | **Multer** | Data fully removed after deletion request | List objects by user prefix; expect empty |
| DD-S-009 | **openai** | Data fully removed after deletion request | Query AI logs by user ID; expect no matching entries |
| DD-S-010 | **posthog** | Data fully removed after deletion request | Export user data report; expect no matching records |
| DD-S-011 | **stripe** | Data fully removed after deletion request | Query payment records by user ID; expect empty result |

### 3.3 Cascade Deletion

| ID | Test Case | Expected Result | Regulation |
| --- | --- | --- | --- |
| DD-C-001 | Delete user account | Data removed from all 11 detected services | GDPR Art. 17 |
...
| **@segment/analytics-next** | Required | Required | Required | Recommended |
| **@sendgrid/mail** | Required | Required | Required | Recommended |
| **algoliasearch** | N/A | Required | Required | Recommended |
| **cookie-parser** | N/A | Required | Required | Recommended |
| **express-session** | N/A | Required | Required | Recommended |
| **ioredis** | N/A | Required | Required | Required |
| **Multer** | N/A | Required | Required | Required |
