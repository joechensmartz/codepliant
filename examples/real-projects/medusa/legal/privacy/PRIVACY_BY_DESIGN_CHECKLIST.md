...
| @segment/analytics-next | analytics | user identity, user behavior, page views, custom events, device information, IP address | Consent mechanism; IP anonymization; retention review |
| @sendgrid/mail | email | email addresses, email content | Consent/opt-out mechanism; DPA with provider; retention policy |
| algoliasearch | other | search queries, search result clicks, user search behavior | Review data processing scope; assess DPA requirements |
| cookie-parser | other | cookies, cookie data | Review data processing scope; assess DPA requirements |
| express-session | other | session cookies, session data | Review data processing scope; assess DPA requirements |
| ioredis | database | cached data, session data | Encryption at rest; access controls; backup encryption |
| Multer | storage | uploaded files, file metadata, potential PII in uploaded content | Encryption review; lifecycle policies; access controls |
