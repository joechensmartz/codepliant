...
> 11 services | Generated on 2026-03-16
...
| @segment/analytics-next | analytics | **P1** | **P3** | user identity, user behavior, page views | ePrivacy, GDPR Art. 6/7 (consent) |
| @sendgrid/mail | email | **P1** | **P2** | email addresses, email content | CAN-SPAM, GDPR Art. 33/34 |
| algoliasearch | other | **P2** | **P3** | search queries, search result clicks, user search behavior | Assess per data processed |
| cookie-parser | other | **P2** | **P3** | cookies, cookie data | Assess per data processed |
| express-session | other | **P2** | **P3** | session cookies, session data | Assess per data processed |
| ioredis | database | **P0** | **P1** | cached data, session data | GDPR Art. 33/34, data breach notification |
| Multer | storage | **P1** | **P2** | uploaded files, file metadata, potential PII in uploaded content | GDPR Art. 33/34 if PII stored |
...
*This incident severity matrix was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on an automated scan of the **root** codebase (11 services detected). Customize response times, escalation paths, and roles to match your organization's structure. This document should be reviewed by your security team and legal counsel before adoption.*
