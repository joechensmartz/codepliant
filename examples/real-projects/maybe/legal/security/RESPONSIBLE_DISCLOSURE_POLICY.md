...
| Authentication system (rails-sessions) | Auth Infrastructure | Critical |
| Payment processing (plaid, stripe) | Payment Infrastructure | Critical |
| Data storage (Active Storage, ActiveRecord, ActiveStorage, aws-sdk-s3, pg, rails-activerecord, redis) | Data Infrastructure | High |
...
- **Authentication & Authorization** — bypass, privilege escalation, session management flaws (rails-sessions)
- **Payment Security** — transaction manipulation, price tampering, payment data exposure (plaid, stripe)
- **Data Storage** — injection, unauthorized data access, data leakage (Active Storage, ActiveRecord, ActiveStorage, aws-sdk-s3, pg, rails-activerecord, redis)
