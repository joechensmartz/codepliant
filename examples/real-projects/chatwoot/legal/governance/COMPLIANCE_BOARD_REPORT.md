...
[Your Company Name] operates a technology stack comprising **32 third-party services** across 10 categories. This report provides an overview of our compliance posture, key risks, regulatory developments, and budget status for Q1 2026.
...
| **Data Processing Categories** | 10 |
| **Data Categories Detected** | 12 |
...
| Storage | 5 | Medium | Vendor dependency, data processing |
| Other | 6 | Medium | Vendor dependency, data processing |
| Email | 4 | Medium | Vendor dependency, data processing |
| Database | 8 | Medium | Vendor dependency, data processing |
| Advertising | 1 | Medium | Vendor dependency, data processing |
...
3. **Vendor consolidation review** — With 32 third-party services, evaluate opportunities to reduce vendor footprint and associated compliance overhead.

4. **Legal review cycle** — Schedule annual legal review of all auto-generated compliance documents to ensure they reflect current business practices and regulatory requirements.

5. **Board compliance training** — Consider annual board-level compliance awareness briefing to maintain governance effectiveness.
...
|---|---------|----------|----------------|----------------|
| 1 | @amplitude/analytics-browser | analytics | Yes | user behavior, device information, session data |
| 2 | @aws-sdk/client-s3 | storage | Yes | uploaded files, file metadata |
| 3 | @twilio/voice-sdk | other | Yes | phone numbers, voice call metadata, call recordings +1 more |
| 4 | ActionCable | other | Yes | real-time user data, connection metadata, channel subscriptions +1 more |
| 5 | ActionController::Cookies | other | Yes | session cookies, session data, CSRF tokens |
| 6 | ActionMailer | email | Yes | email addresses, email content |
| 7 | Active Storage | storage | Yes | uploaded files, file metadata, storage service credentials +1 more |
| 8 | ActiveRecord | database | Yes | user data as defined in schema, timestamps, associations |
| 9 | ActiveStorage | storage | Yes | uploaded files, file metadata, storage references |
| 10 | aws-sdk-s3 | storage | Yes | uploaded files, file metadata |
| 11 | devise | auth | Yes | email, password hash, session data +1 more |
| 12 | google-cloud-storage | storage | Yes | uploaded files, file metadata |
| 13 | ioredis | database | Yes | cached data, session data |
| 14 | MailHog | email | Yes | email content |
| 15 | Meta Pixel | advertising | Yes | page views, conversion events, user behavior +1 more |
| 16 | nodemailer | email | Yes | email addresses, email content |
| 17 | omniauth | auth | Yes | email, name, OAuth tokens +1 more |
| 18 | pg | database | Yes | user data as defined in schema |
| 19 | PostgreSQL (env) | database | Yes | application data, user records |
| 20 | pundit | auth | Yes | user roles, authorization policies, access control data |
| 21 | rack-attack | other | Yes | IP addresses, request metadata |
| 22 | rails-actionmailer | email | Yes | email addresses, email content |
| 23 | rails-activerecord | database | Yes | user data as defined in schema |
| 24 | rails-sessions | auth | Yes | session cookies, CSRF tokens |
| 25 | redis | database | Yes | cached data, session data |
| | *... and 7 more services* | | | |

---

*This board report was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on automated source code analysis of the **@chatwoot/chatwoot** codebase (32 services detected). Sections marked with [brackets] require manual input from the compliance team. This report should be reviewed and supplemented with operational data before board presentation. Generated on 2026-03-16.*
