...
**DPO / Privacy Contact:** [dpo@example.com]
**Created:** 2026-03-16
...
|------|------|-----------------|-------------|
| ACC | Access (Art. 15) | 30 | Subject requests copy of their personal data |
| ERA | Erasure (Art. 17) | 30 | Subject requests deletion of their personal data |
| REC | Rectification (Art. 16) | 30 | Subject requests correction of inaccurate data |
| POR | Portability (Art. 20) | 30 | Subject requests data in machine-readable format |
| RES | Restriction (Art. 18) | 30 | Subject requests restriction of processing |
| OBJ | Objection (Art. 21) | 30 | Subject objects to processing |
...
|---------|----------|----------------|-----------------|
| @amplitude/analytics-browser | analytics | user behavior, device information, session data | Export / Delete / Rectify |
| @aws-sdk/client-s3 | storage | uploaded files, file metadata | Export / Delete / Rectify |
| @twilio/voice-sdk | other | phone numbers, voice call metadata, call recordings, device information | Export / Delete / Rectify |
| ActionCable | other | real-time user data, connection metadata, channel subscriptions, WebSocket messages | Export / Delete / Rectify |
| ActionController::Cookies | other | session cookies, session data, CSRF tokens | Export / Delete / Rectify |
| ActionMailer | email | email addresses, email content | Export / Delete / Rectify |
| Active Storage | storage | uploaded files, file metadata, storage service credentials, potential PII in uploaded content | Export / Delete / Rectify |
| ActiveRecord | database | user data as defined in schema, timestamps, associations | Export / Delete / Rectify |
| ActiveStorage | storage | uploaded files, file metadata, storage references | Export / Delete / Rectify |
| aws-sdk-s3 | storage | uploaded files, file metadata | Export / Delete / Rectify |
| devise | auth | email, password hash, session data, authentication tokens | Export / Delete / Rectify |
| google-cloud-storage | storage | uploaded files, file metadata | Export / Delete / Rectify |
| ioredis | database | cached data, session data | Export / Delete / Rectify |
| MailHog | email | email content | Export / Delete / Rectify |
| Meta Pixel | advertising | page views, conversion events, user behavior, device information | Export / Delete / Rectify |
| nodemailer | email | email addresses, email content | Export / Delete / Rectify |
| omniauth | auth | email, name, OAuth tokens, profile data | Export / Delete / Rectify |
| pg | database | user data as defined in schema | Export / Delete / Rectify |
| PostgreSQL (env) | database | application data, user records | Export / Delete / Rectify |
| pundit | auth | user roles, authorization policies, access control data | Export / Delete / Rectify |
| rack-attack | other | IP addresses, request metadata | Export / Delete / Rectify |
| rails-actionmailer | email | email addresses, email content | Export / Delete / Rectify |
| rails-activerecord | database | user data as defined in schema | Export / Delete / Rectify |
| rails-sessions | auth | session cookies, CSRF tokens | Export / Delete / Rectify |
| redis | database | cached data, session data | Export / Delete / Rectify |
| Redis | database | session data, cache data | Export / Delete / Rectify |
| Redis (env) | database | session data, cache data | Export / Delete / Rectify |
| ruby-openai | ai | user prompts, conversation history, generated content | Export / Delete / Rectify |
| sentry-ruby | monitoring | error data, stack traces, user context, device information | Export / Delete / Rectify |
| sidekiq | other | job data, user data processed in background jobs | Export / Delete / Rectify |
| stripe | payment | payment information, billing address, email, transaction history | Export / Delete / Rectify |
| twilio-ruby | other | phone numbers, SMS message content, voice call metadata | Export / Delete / Rectify |
