...
This Encryption Policy defines the encryption requirements for data at rest, data in transit, and key management procedures for **[Your Company Name]**'s **@chatwoot/chatwoot** application. It supports compliance with GDPR Article 32, SOC 2 CC6.1, ISO 27001 Annex A.10, and PCI DSS Requirement 3 & 4.
...
| @aws-sdk/client-s3 (Storage) | AES-256 server-side encryption (SSE-S3, SSE-KMS, or SSE-C for AWS S3 | Cloud-managed keys | Cloud-managed or CMEK |
...
| ActiveStorage (Storage) | AES-256 server-side encryption (SSE-S3, SSE-KMS, or SSE-C for AWS S3 | Cloud-managed keys | Cloud-managed or CMEK |
| aws-sdk-s3 (Storage) | AES-256 server-side encryption (SSE-S3, SSE-KMS, or SSE-C for AWS S3 | Cloud-managed keys | Cloud-managed or CMEK |
| google-cloud-storage (Storage) | AES-256 server-side encryption (SSE-S3, SSE-KMS, or SSE-C for AWS S3 | Cloud-managed keys | Cloud-managed or CMEK |
...
| Session data | AES-256 at rest | Short TTL; encrypted cookies with Secure + HttpOnly flags |
| OAuth tokens | AES-256 at rest | Encrypt in database; never log token values |
...
| Provider | At-Rest Encryption | In-Transit Encryption | Key Management |
|----------|-------------------|----------------------|----------------|
| Amazon Web Services (AWS) | AES-256 (SSE-S3, SSE-KMS, SSE-C) | TLS 1.2+ enforced | AWS KMS (AWS-managed or CMK) |
...
*This Encryption Policy was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on an automated scan of the **@chatwoot/chatwoot** codebase. It should be reviewed by your security and engineering teams and customized to reflect your actual encryption implementation.*