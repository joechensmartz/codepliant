...
This Business Continuity Plan (BCP) defines recovery objectives, failover procedures, and communication protocols for **[Your Company Name]**'s **@chatwoot/chatwoot** application. It ensures that critical services can be restored within acceptable timeframes following an outage, disaster, or other disruptive event.
...
|---------|----------|-----|-----|-------------------|
| devise | Authentication | 1 hour | 0 minutes | Session cache failover; extend existing tokens |
| omniauth | Authentication | 1 hour | 0 minutes | Session cache failover; extend existing tokens |
| pundit | Authentication | 1 hour | 0 minutes | Session cache failover; extend existing tokens |
| rails-sessions | Authentication | 1 hour | 0 minutes | Session cache failover; extend existing tokens |
| stripe | Payment Processing | 1 hour | 0 minutes | Transaction queue with retry; idempotency keys |
| ActiveRecord | Database | 1 hour | 5 minutes | Automated replica promotion; point-in-time recovery |
| ioredis | Database | 1 hour | 5 minutes | Automated replica promotion; point-in-time recovery |
| pg | Database | 1 hour | 5 minutes | Automated replica promotion; point-in-time recovery |
| PostgreSQL (env) | Database | 1 hour | 5 minutes | Automated replica promotion; point-in-time recovery |
| rails-activerecord | Database | 1 hour | 5 minutes | Automated replica promotion; point-in-time recovery |
...
| Service | Category | RTO | RPO | Recovery Strategy |
|---------|----------|-----|-----|-------------------|
| ActionMailer | Email | 4 hours | 1 hour | Queue with retry; alternative SMTP provider |
| MailHog | Email | 4 hours | 1 hour | Queue with retry; alternative SMTP provider |
| nodemailer | Email | 4 hours | 1 hour | Queue with retry; alternative SMTP provider |
| rails-actionmailer | Email | 4 hours | 1 hour | Queue with retry; alternative SMTP provider |
| @aws-sdk/client-s3 | Storage | 4 hours | 1 hour | Cross-region replication; CDN fallback |
| Active Storage | Storage | 4 hours | 1 hour | Cross-region replication; CDN fallback |
| ActiveStorage | Storage | 4 hours | 1 hour | Cross-region replication; CDN fallback |
| aws-sdk-s3 | Storage | 4 hours | 1 hour | Cross-region replication; CDN fallback |
| google-cloud-storage | Storage | 4 hours | 1 hour | Cross-region replication; CDN fallback |
| ruby-openai | AI Service | 4 hours | 1 hour | Graceful degradation; disable AI features |
...
| Service | Category | RTO | RPO | Recovery Strategy |
|---------|----------|-----|-----|-------------------|
| sentry-ruby | Monitoring | 24 hours | 24 hours | Secondary monitoring; manual checks |
| @amplitude/analytics-browser | Analytics | 24 hours | 24 hours | Data buffering; delayed processing |
| Meta Pixel | Analytics | 24 hours | 24 hours | Graceful degradation; static fallback |

## 2. Infrastructure Overview

### 2.1 Cloud Providers

| Provider | Detected Region(s) | Data Residency |
|----------|--------------------|----------------|
| Amazon Web Services (AWS) | Not specified | AWS operates data centers in multiple regions worldwide |
...
Detected database service(s): ActiveRecord, ioredis, pg, PostgreSQL (env), rails-activerecord, redis, Redis, Redis (env)
...
Detected auth service(s): devise, omniauth, pundit, rails-sessions
...
Detected AI service(s): ruby-openai
...
|---------|----------|----------------------|--------------------------|
| @amplitude/analytics-browser | analytics | Usage data not collected (no user impact) | Buffer events client-side |
| @aws-sdk/client-s3 | storage | File uploads/downloads unavailable | CDN cache; deferred uploads |
| @twilio/voice-sdk | other | Feature degradation | Evaluate per service |
| ActionCable | other | Feature degradation | Evaluate per service |
| ActionController::Cookies | other | Feature degradation | Evaluate per service |
| ActionMailer | email | Transactional emails delayed or undelivered | Alternative email provider; manual send |
| Active Storage | storage | File uploads/downloads unavailable | CDN cache; deferred uploads |
| ActiveRecord | database | Data reads/writes unavailable | Read replica; cached responses |
| ActiveStorage | storage | File uploads/downloads unavailable | CDN cache; deferred uploads |
| aws-sdk-s3 | storage | File uploads/downloads unavailable | CDN cache; deferred uploads |
| devise | auth | Users cannot sign in or register | Cached sessions; manual verification |
| google-cloud-storage | storage | File uploads/downloads unavailable | CDN cache; deferred uploads |
| ioredis | database | Data reads/writes unavailable | Read replica; cached responses |
| MailHog | email | Transactional emails delayed or undelivered | Alternative email provider; manual send |
| Meta Pixel | advertising | Ad tracking paused (no user impact) | Pause campaigns; no data loss |
| nodemailer | email | Transactional emails delayed or undelivered | Alternative email provider; manual send |
| omniauth | auth | Users cannot sign in or register | Cached sessions; manual verification |
| pg | database | Data reads/writes unavailable | Read replica; cached responses |
| PostgreSQL (env) | database | Data reads/writes unavailable | Read replica; cached responses |
| pundit | auth | Users cannot sign in or register | Cached sessions; manual verification |
| rack-attack | other | Feature degradation | Evaluate per service |
| rails-actionmailer | email | Transactional emails delayed or undelivered | Alternative email provider; manual send |
| rails-activerecord | database | Data reads/writes unavailable | Read replica; cached responses |
| rails-sessions | auth | Users cannot sign in or register | Cached sessions; manual verification |
...
| ruby-openai | ai | AI-powered features unavailable | Disable AI features; show static content |
| sentry-ruby | monitoring | Blind to errors and performance issues | Manual log review; secondary tools |
| sidekiq | other | Feature degradation | Evaluate per service |
| stripe | payment | Payment processing halted | Queue transactions for later processing |
| twilio-ruby | other | Feature degradation | Evaluate per service |
...
*This Business Continuity Plan was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on an automated scan of the **@chatwoot/chatwoot** codebase. Recovery objectives and procedures should be reviewed and customized by your engineering and operations teams based on actual infrastructure and business requirements.*