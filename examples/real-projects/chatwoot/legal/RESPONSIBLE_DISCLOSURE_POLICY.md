...
| Authentication system (devise, omniauth, pundit, rails-sessions) | Auth Infrastructure | Critical |
| Payment processing (stripe) | Payment Infrastructure | Critical |
| Data storage (@aws-sdk/client-s3, Active Storage, ActiveRecord, ActiveStorage, aws-sdk-s3, google-cloud-storage, ioredis, pg, PostgreSQL (env), rails-activerecord, redis, Redis, Redis (env)) | Data Infrastructure | High |
...
- **Authentication & Authorization** — bypass, privilege escalation, session management flaws (devise, omniauth, pundit, rails-sessions)
- **Payment Security** — transaction manipulation, price tampering, payment data exposure (stripe)
- **Data Storage** — injection, unauthorized data access, data leakage (@aws-sdk/client-s3, Active Storage, ActiveRecord, ActiveStorage, aws-sdk-s3, google-cloud-storage, ioredis, pg, PostgreSQL (env), rails-activerecord, redis, Redis, Redis (env))
