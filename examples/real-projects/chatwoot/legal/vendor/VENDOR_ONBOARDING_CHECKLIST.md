...
|---|--------|----------|-----------|-------------------|
| 1 | @amplitude/analytics-browser | Analytics | Medium | Internal — collects behavioral data, device info, IP addresses |
| 2 | @aws-sdk/client-s3 | File Storage | High | Confidential — stores files that may contain any data category |
| 3 | @twilio/voice-sdk | Other | Low | Internal — classification depends on specific use case |
| 4 | ActionCable | Other | Low | Internal — classification depends on specific use case |
| 5 | ActionController::Cookies | Other | Low | Internal — classification depends on specific use case |
| 6 | ActionMailer | Email Service | Medium | Confidential — processes email addresses, message content |
| 7 | Active Storage | File Storage | High | Confidential — stores files that may contain any data category |
| 8 | ActiveRecord | Database | High | Confidential — stores application data, potentially all data categories |
| 9 | ActiveStorage | File Storage | High | Confidential — stores files that may contain any data category |
| 10 | aws-sdk-s3 | File Storage | High | Confidential — stores files that may contain any data category |
| 11 | devise | Authentication | High | Confidential — processes authentication credentials, identity data |
| 12 | google-cloud-storage | File Storage | High | Confidential — stores files that may contain any data category |
| 13 | ioredis | Database | High | Confidential — stores application data, potentially all data categories |
| 14 | MailHog | Email Service | Medium | Confidential — processes email addresses, message content |
| 15 | Meta Pixel | Advertising | Medium | Internal — processes device IDs, behavioral data, ad interactions |
| 16 | nodemailer | Email Service | Medium | Confidential — processes email addresses, message content |
| 17 | omniauth | Authentication | High | Confidential — processes authentication credentials, identity data |
| 18 | pg | Database | High | Confidential — stores application data, potentially all data categories |
| 19 | PostgreSQL (env) | Database | High | Confidential — stores application data, potentially all data categories |
| 20 | pundit | Authentication | High | Confidential — processes authentication credentials, identity data |
| 21 | rack-attack | Other | Low | Internal — classification depends on specific use case |
| 22 | rails-actionmailer | Email Service | Medium | Confidential — processes email addresses, message content |
| 23 | rails-activerecord | Database | High | Confidential — stores application data, potentially all data categories |
| 24 | rails-sessions | Authentication | High | Confidential — processes authentication credentials, identity data |
| 25 | redis | Database | High | Confidential — stores application data, potentially all data categories |
| 26 | Redis | Database | High | Confidential — stores application data, potentially all data categories |
| 27 | Redis (env) | Database | High | Confidential — stores application data, potentially all data categories |
| 28 | ruby-openai | AI Service | Critical | Confidential — may process user content, prompts, PII |
| 29 | sentry-ruby | Error Monitoring | Medium | Internal — collects error logs, stack traces, device metadata |
| 30 | sidekiq | Other | Low | Internal — classification depends on specific use case |
| 31 | stripe | Payment Processing | Critical | Restricted — processes payment card data, financial PII |
| 32 | twilio-ruby | Other | Low | Internal — classification depends on specific use case |

> **Total vendors:** 32 | **Critical/High risk:** 19
...
*This Vendor Onboarding Checklist was auto-generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on a scan of the project's codebase. It should be reviewed by your legal and procurement teams before use.*