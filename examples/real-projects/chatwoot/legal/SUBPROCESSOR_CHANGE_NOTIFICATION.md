...
|--------------|---------|---------------|
| Amplitude | Product analytics and usage tracking | user behavior, device information, session data |
| Amazon S3 (AWS) | File storage and media hosting | uploaded files, file metadata |
| Twilio | Third-party service integration | phone numbers, voice call metadata, call recordings, device information |
...
| ActionController::Cookies | Third-party service integration | session cookies, session data, CSRF tokens |
| ActionMailer | Transactional and marketing email delivery | email addresses, email content |
...
| ActiveRecord | Data storage and management | user data as defined in schema, timestamps, associations |
| ActiveStorage | File storage and media hosting | uploaded files, file metadata, storage references |
| aws-sdk-s3 | File storage and media hosting | uploaded files, file metadata |
| devise | User authentication and identity management | email, password hash, session data, authentication tokens |
| google-cloud-storage | File storage and media hosting | uploaded files, file metadata |
| MailHog | Transactional and marketing email delivery | email content |
| Meta Pixel | Advertising and ad measurement | page views, conversion events, user behavior, device information |
| omniauth | User authentication and identity management | email, name, OAuth tokens, profile data |
| pg | Data storage and management | user data as defined in schema |
| PostgreSQL (env) | Data storage and management | application data, user records |
| pundit | User authentication and identity management | user roles, authorization policies, access control data |
| rack-attack | Third-party service integration | IP addresses, request metadata |
| rails-actionmailer | Transactional and marketing email delivery | email addresses, email content |
| rails-activerecord | Data storage and management | user data as defined in schema |
| rails-sessions | User authentication and identity management | session cookies, CSRF tokens |
| Redis | Data storage and management | session data, cache data |
| Redis (env) | Data storage and management | session data, cache data |
| ruby-openai | AI processing and content generation | user prompts, conversation history, generated content |
| sentry-ruby | Error tracking and performance monitoring | error data, stack traces, user context, device information |
| sidekiq | Third-party service integration | job data, user data processed in background jobs |
| Stripe | Payment processing and billing | payment information, billing address, email, transaction history |
| twilio-ruby | Third-party service integration | phone numbers, SMS message content, voice call metadata |
...
*This Sub-Processor Change Notification template was auto-generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on code analysis. Complete the bracketed fields before sending to customers. Review with legal counsel to ensure compliance with your specific DPA obligations.*
