...
> 32 services | Generated on 2026-03-16
...
|---------|----------|---------------|-----------|-------------|-------------------|
| @amplitude/analytics-browser | analytics | **P1** | **P3** | user behavior, device information, session data | ePrivacy, GDPR Art. 6/7 (consent) |
| @aws-sdk/client-s3 | storage | **P1** | **P2** | uploaded files, file metadata | GDPR Art. 33/34 if PII stored |
| @twilio/voice-sdk | other | **P2** | **P3** | phone numbers, voice call metadata, call recordings | Assess per data processed |
...
| google-cloud-storage | storage | **P1** | **P2** | uploaded files, file metadata | GDPR Art. 33/34 if PII stored |
| ioredis | database | **P0** | **P1** | cached data, session data | GDPR Art. 33/34, data breach notification |
| MailHog | email | **P1** | **P2** | email content | CAN-SPAM, GDPR Art. 33/34 |
| Meta Pixel | advertising | **P2** | **P3** | page views, conversion events, user behavior | Assess per data processed |
...
| ruby-openai | ai | **P0** | **P2** | user prompts, conversation history, generated content | EU AI Act, GDPR Art. 22 (automated decisions) |
| sentry-ruby | monitoring | **P2** | **P2** | error data, stack traces, user context | Minimal direct regulatory impact |
...
| twilio-ruby | other | **P2** | **P3** | phone numbers, SMS message content, voice call metadata | Assess per data processed |
...
*This incident severity matrix was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on an automated scan of the **@chatwoot/chatwoot** codebase (32 services detected). Customize response times, escalation paths, and roles to match your organization's structure. This document should be reviewed by your security team and legal counsel before adoption.*
