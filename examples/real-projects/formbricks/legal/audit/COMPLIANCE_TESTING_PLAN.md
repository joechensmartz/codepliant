...
| DD-S-002 | **@sentry/nextjs** | Data fully removed after deletion request | Search logs by user ID; expect scrubbed entries |
| DD-S-003 | **googleapis** | Data fully removed after deletion request | Service-specific API query; expect no user data |
| DD-S-004 | **ioredis** | Data fully removed after deletion request | Direct DB query by user ID; expect 0 rows |
| DD-S-005 | **next-auth** | Data fully removed after deletion request | Query user profile API; expect 404 |
| DD-S-006 | **nodemailer** | Data fully removed after deletion request | Search mailing lists by email; expect not found |
| DD-S-007 | **posthog** | Data fully removed after deletion request | Export user data report; expect no matching records |
| DD-S-008 | **prisma** | Data fully removed after deletion request | Direct DB query by user ID; expect 0 rows |
| DD-S-009 | **redis** | Data fully removed after deletion request | Direct DB query by user ID; expect 0 rows |
| DD-S-010 | **stripe** | Data fully removed after deletion request | Query payment records by user ID; expect empty result |

### 3.3 Cascade Deletion

| ID | Test Case | Expected Result | Regulation |
| --- | --- | --- | --- |
| DD-C-001 | Delete user account | Data removed from all 10 detected services | GDPR Art. 17 |
