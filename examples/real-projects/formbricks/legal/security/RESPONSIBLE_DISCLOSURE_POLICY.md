...
| Authentication system (next-auth) | Auth Infrastructure | Critical |
| Payment processing (stripe) | Payment Infrastructure | Critical |
| Data storage (@aws-sdk/client-s3, ioredis, prisma, redis) | Data Infrastructure | High |
...
- **Authentication & Authorization** — bypass, privilege escalation, session management flaws (next-auth)
- **Payment Security** — transaction manipulation, price tampering, payment data exposure (stripe)
- **Data Storage** — injection, unauthorized data access, data leakage (@aws-sdk/client-s3, ioredis, prisma, redis)
