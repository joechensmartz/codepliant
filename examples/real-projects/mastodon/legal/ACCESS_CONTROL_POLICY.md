...
|---------|---------------|
| devise | email, password hash, session data, authentication tokens |
| omniauth | email, name, OAuth tokens, profile data |
| pundit | user roles, authorization policies, access control data |
| rails-sessions | session cookies, CSRF tokens |
...
Consider integrating TOTP-based MFA (e.g., via devise) as a first step.
