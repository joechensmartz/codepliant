...
|-------------|----------------------|-------------------|
| @ai-sdk/google-vertex | Unknown | Review provider policy |
...
- **@ai-sdk/google-vertex** — detected in `package.json`, `packages/lib/server-only/ai/google.ts`, `package.json (root)`, `apps/docs/package.json`, `apps/openpage-api/package.json`, `apps/remix/package.json`, `packages/api/package.json`, `packages/app-tests/package.json`, `packages/assets/package.json`, `packages/auth/package.json`, `packages/ee/package.json`, `packages/email/package.json`, `packages/eslint-config/package.json`, `packages/lib/package.json`, `packages/lib/server-only/ai/google.ts`, `packages/prettier-config/package.json`, `packages/prisma/package.json`, `packages/signing/package.json`, `packages/tailwind-config/package.json`, `packages/trpc/package.json`, `packages/tsconfig/package.json`, `packages/ui/package.json`
- **@vercel/ai** — detected in `packages/lib/server-only/ai/envelope/detect-fields/index.ts`, `packages/lib/server-only/ai/envelope/detect-recipients/index.ts`, `packages/lib/server-only/ai/envelope/detect-fields/index.ts`, `packages/lib/server-only/ai/envelope/detect-recipients/index.ts`
...
*This AI training data notice was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on an automated scan of the **@documenso/root** codebase. AI provider training policies are subject to change. This document should be reviewed regularly and verified against each provider's current terms of service and privacy policy.*