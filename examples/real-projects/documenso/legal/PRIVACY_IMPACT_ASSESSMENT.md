...
This assessment covers the data processing activities of the **@documenso/root** application operated by [Your Company Name]. The following describes the nature, scope, context, and purposes of processing.
...
|---------|----------|---------------|-------------|
| @ai-sdk/google-vertex | ai | user prompts, conversation history, generated content | Consent / Legitimate interest |
| @aws-sdk/client-ses | email | email addresses, email content, uploaded files, file metadata | Contractual necessity / Consent |
| @google-cloud/kms | other | encryption keys, key metadata | Legitimate interest |
| @simplewebauthn/server | auth | biometric authentication data, device attestation, credential IDs | Contractual necessity |
...
- **Personal Identity Data:** Email addresses, names, profile pictures, and account credentials collected through authentication. (sources: @simplewebauthn/server, next-auth, passport-microsoft)
...
- **AI Interaction Data:** User prompts, conversation history, and AI-generated content processed through third-party AI services. (sources: @ai-sdk/google-vertex, @vercel/ai)
- **Communication Data:** Email addresses and email content processed through email service providers. (sources: nodemailer, @aws-sdk/client-ses, resend)
- **Stored User Data:** Persistent user data stored in databases as defined by the application schema. (sources: prisma)
...
|---------------------|-------------|---------------|
| AI processing via @ai-sdk/google-vertex | Consent / Legitimate interest | Required for AI-powered features; user consent obtained before processing |
| Email communications via @aws-sdk/client-ses | Contractual necessity / Consent | Necessary for service communications; marketing requires separate consent |
| Data processing via @google-cloud/kms | Legitimate interest | To be documented based on specific use case |
| Authentication via @simplewebauthn/server | Contractual necessity | Necessary to authenticate and maintain user sessions |
...
|---|---------------------|---------------|------------|--------|-------|--------|
| 1 | AI processing via @ai-sdk/google-vertex | user prompts, conversation history, generated content | 3 (Likely) | 2 (Limited) | **6** | **Medium** |
| 2 | Email communications via @aws-sdk/client-ses | email addresses, email content, uploaded files, file metadata | 2 (Possible) | 2 (Limited) | **4** | **Low** |
| 3 | Data processing via @google-cloud/kms | encryption keys, key metadata | 1 (Unlikely) | 1 (Negligible) | **1** | **Low** |
| 4 | Authentication via @simplewebauthn/server | biometric authentication data, device attestation, credential IDs | 2 (Possible) | 4 (Maximum) | **8** | **Medium** |
| 5 | AI processing via @vercel/ai | user prompts, conversation history, generated content | 3 (Likely) | 2 (Limited) | **6** | **Medium** |
| 6 | Data processing via googleapis | user data via Google APIs, calendar data, email data, profile information | 1 (Unlikely) | 2 (Limited) | **2** | **Low** |
| 7 | Authentication via next-auth | email, name, profile picture, OAuth tokens, session data | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
| 8 | Email communications via nodemailer | email addresses, email content | 2 (Possible) | 2 (Limited) | **4** | **Low** |
| 9 | Authentication via passport-microsoft | email, name, Microsoft profile data, OAuth tokens | 2 (Possible) | 3 (Significant) | **6** | **Medium** |
...
| 11 | Data persistence via prisma | user data as defined in schema | 2 (Possible) | 1 (Negligible) | **2** | **Low** |
| 12 | Email communications via resend | email addresses, email content | 2 (Possible) | 2 (Limited) | **4** | **Low** |
| 13 | Payment processing via stripe | payment information, billing address, email, transaction history | 2 (Possible) | 4 (Maximum) | **8** | **Medium** |
...
| Sensitive / Special Category Data | **TRIGGERED** | Processing of special categories of data on a large scale (Art. 35(3)(b)) |
...
> **4 high-risk trigger(s) detected.** This DPIA is mandatory under Article 35 GDPR. Multiple triggers increase the overall risk profile.
...
### 6.1 Ai (@ai-sdk/google-vertex, @vercel/ai)
...
### 6.2 Email (@aws-sdk/client-ses, nodemailer, resend)
...
### 6.3 Other (@google-cloud/kms, googleapis)
...
### 6.4 Auth (@simplewebauthn/server, next-auth, passport-microsoft)
