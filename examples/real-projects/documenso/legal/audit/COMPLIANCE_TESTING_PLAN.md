...
| --- | --- | --- | --- |
| DD-S-001 | **@ai-sdk/google-vertex** | Data fully removed after deletion request | Query AI logs by user ID; expect no matching entries |
| DD-S-002 | **@aws-sdk/client-ses** | Data fully removed after deletion request | Search mailing lists by email; expect not found |
| DD-S-003 | **@google-cloud/kms** | Data fully removed after deletion request | Service-specific API query; expect no user data |
| DD-S-004 | **@simplewebauthn/server** | Data fully removed after deletion request | Query user profile API; expect 404 |
| DD-S-005 | **@vercel/ai** | Data fully removed after deletion request | Query AI logs by user ID; expect no matching entries |
| DD-S-006 | **googleapis** | Data fully removed after deletion request | Service-specific API query; expect no user data |
| DD-S-007 | **next-auth** | Data fully removed after deletion request | Query user profile API; expect 404 |
| DD-S-008 | **nodemailer** | Data fully removed after deletion request | Search mailing lists by email; expect not found |
| DD-S-009 | **passport-microsoft** | Data fully removed after deletion request | Query user profile API; expect 404 |
| DD-S-010 | **posthog** | Data fully removed after deletion request | Export user data report; expect no matching records |
| DD-S-011 | **prisma** | Data fully removed after deletion request | Direct DB query by user ID; expect 0 rows |
| DD-S-012 | **resend** | Data fully removed after deletion request | Search mailing lists by email; expect not found |
| DD-S-013 | **stripe** | Data fully removed after deletion request | Query payment records by user ID; expect empty result |

### 3.3 Cascade Deletion

| ID | Test Case | Expected Result | Regulation |
| --- | --- | --- | --- |
| DD-C-001 | Delete user account | Data removed from all 13 detected services | GDPR Art. 17 |
...
- **@simplewebauthn/server**: Verify MFA enforcement for admin accounts
- **@simplewebauthn/server**: Verify OAuth scope restrictions match configured minimums
- **@simplewebauthn/server**: Verify session token rotation on privilege escalation
...
| --- | --- | --- | --- | --- |
| **@ai-sdk/google-vertex** | Required | Required | Required | Recommended |
| **@aws-sdk/client-ses** | Required | Required | Required | Recommended |
| **@google-cloud/kms** | N/A | Required | Required | Recommended |
| **@simplewebauthn/server** | N/A | Required | Required | Required |
...
| **resend** | Required | Required | Required | Recommended |
