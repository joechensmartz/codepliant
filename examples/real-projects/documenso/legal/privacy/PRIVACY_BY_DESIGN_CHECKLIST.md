...
| --- | --- | --- | --- |
| @ai-sdk/google-vertex | ai | user prompts, conversation history, generated content | DPIA required; document AI decision logic; assess bias risks |
| @aws-sdk/client-ses | email | email addresses, email content, uploaded files, file metadata | Consent/opt-out mechanism; DPA with provider; retention policy |
| @google-cloud/kms | other | encryption keys, key metadata | Review data processing scope; assess DPA requirements |
| @simplewebauthn/server | auth | biometric authentication data, device attestation, credential IDs | Minimize data collected; secure credential storage; session management |
...
| resend | email | email addresses, email content | Consent/opt-out mechanism; DPA with provider; retention policy |
