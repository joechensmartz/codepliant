...
|---------|----------|--------------|------------|
| `@ai-sdk/anthropic` | ai | user prompts, conversation history, generated content | Medium |
| `@ai-sdk/google` | ai | user prompts, conversation history, generated content | Medium |
| `@ai-sdk/openai` | ai | user prompts, conversation history, generated content | Medium |
| `@vercel/ai` | ai | user prompts, conversation history, generated content | Medium |
| `openai` | ai | user prompts, conversation history, generated content | Medium |

> **Scope:** This guide covers adversarial testing for 5 detected AI service(s). Each service should be tested against all applicable scenarios below.
...
### Vercel AI SDK-Specific Tests

1. Test streaming responses for partial injection during generation
2. Verify tool call results are validated before use
3. Test multi-provider fallback for inconsistent safety behaviour
4. Verify rate limiting across all configured providers
5. Test structured output generation for schema bypass
...
*This AI Red Team Guide was generated by [Codepliant](https://github.com/joechensmartz/codepliant) based on automated code analysis. Test cases are derived from the OWASP Top 10 for LLM Applications (2025) and industry best practices. This guide should be reviewed and customised by your security team before conducting red team exercises. Always obtain proper authorisation before testing.*