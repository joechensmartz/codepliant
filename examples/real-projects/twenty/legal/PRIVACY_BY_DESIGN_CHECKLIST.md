...
| --- | --- | --- | --- |
| @ai-sdk/anthropic | ai | user prompts, conversation history, generated content | DPIA required; document AI decision logic; assess bias risks |
| @ai-sdk/google | ai | user prompts, conversation history, generated content | DPIA required; document AI decision logic; assess bias risks |
| @ai-sdk/openai | ai | user prompts, conversation history, generated content | DPIA required; document AI decision logic; assess bias risks |
| @aws-sdk/client-s3 | storage | uploaded files, file metadata | Encryption review; lifecycle policies; access controls |
| @sentry/node | monitoring | error data, stack traces, user context, device information, IP address, performance profiles | PII scrubbing in error reports; retention limits; access controls |
| @vercel/ai | ai | user prompts, conversation history, generated content | DPIA required; document AI decision logic; assess bias risks |
| drizzle | database | user data as defined in schema | Encryption at rest; access controls; backup encryption |
...
| passport-google-oauth20 | auth | email, name, Google profile data, OAuth tokens | Minimize data collected; secure credential storage; session management |
| passport-microsoft | auth | email, name, Microsoft profile data, OAuth tokens | Minimize data collected; secure credential storage; session management |
