...
| Authentication system (passport, passport-google-oauth20, passport-microsoft) | Auth Infrastructure | Critical |
| Payment processing (stripe) | Payment Infrastructure | Critical |
| Data storage (@aws-sdk/client-s3, drizzle, ioredis, redis) | Data Infrastructure | High |
...
- **Authentication & Authorization** — bypass, privilege escalation, session management flaws (passport, passport-google-oauth20, passport-microsoft)
- **Payment Security** — transaction manipulation, price tampering, payment data exposure (stripe)
- **Data Storage** — injection, unauthorized data access, data leakage (@aws-sdk/client-s3, drizzle, ioredis, redis)
