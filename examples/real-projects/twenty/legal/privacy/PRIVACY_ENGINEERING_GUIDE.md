...
This guide provides actionable engineering patterns for implementing privacy by design across the [Your Company Name] technology stack. Each section is tailored to the 16 service(s) detected in the codebase and includes code examples, configuration recommendations, and testing strategies.
...
- **@sentry/node**: Configure `beforeSend` / scrubbing hooks to strip PII from error payloads
...
- **drizzle**: Enforce SSL/TLS on all connections (`sslmode=require` or equivalent)
...
| --- | --- | --- | --- | --- |
| **@ai-sdk/anthropic** | ai | Strip PII from prompts; disable training on user data | TLS for API calls; encrypt cached prompts/responses | API key scoped per environment; prompt audit logging |
| **@ai-sdk/google** | ai | Strip PII from prompts; disable training on user data | TLS for API calls; encrypt cached prompts/responses | API key scoped per environment; prompt audit logging |
| **@ai-sdk/openai** | ai | Strip PII from prompts; disable training on user data | TLS for API calls; encrypt cached prompts/responses | API key scoped per environment; prompt audit logging |
...
| **@sentry/node** | monitoring | Scrub PII from error payloads and breadcrumbs | TLS for log shipping; encrypt log archives | Read-only dashboards; admin for config changes |
...
| **drizzle** | database | Encrypt PII columns; implement column-level access | TDE or volume encryption; TLS for connections | Per-service DB users; no shared credentials |
...
| **openai** | ai | Strip PII from prompts; disable training on user data | TLS for API calls; encrypt cached prompts/responses | API key scoped per environment; prompt audit logging |
...
| **passport-google-oauth20** | auth | Collect only required profile fields; limit OAuth scopes | Encrypt tokens and credentials at rest; TLS in transit | Admin-only config; audit all auth events |
...
**Detected:** @ai-sdk/anthropic, @ai-sdk/google, @ai-sdk/openai, @vercel/ai, openai
...
async function deleteUserData(userId: string): Promise<DeletionReport> {
  ...
  const deletionTasks = [
    { service: "@ai-sdk/anthropic", fn: () => deleteaisdkanthropicData(userId) },
    { service: "@ai-sdk/google", fn: () => deleteaisdkgoogleData(userId) },
    { service: "@ai-sdk/openai", fn: () => deleteaisdkopenaiData(userId) },
    { service: "@aws-sdk/client-s3", fn: () => deleteawssdkclients3Data(userId) },
    { service: "@sentry/node", fn: () => deletesentrynodeData(userId) },
    { service: "@vercel/ai", fn: () => deletevercelaiData(userId) },
    { service: "drizzle", fn: () => deletedrizzleData(userId) },
    ...
    { service: "openai", fn: () => deleteopenaiData(userId) },
    { service: "passport", fn: () => deletepassportData(userId) },
    { service: "passport-google-oauth20", fn: () => deletepassportgoogleoauth20Data(userId) },
